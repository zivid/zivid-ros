...
**This is a preview of ROS 2 driver for Zivid cameras.**

_Thank you for testing this preview of our ROS2 driver! Be aware that there may be adjustments to behavior and API_
_before the final production release, which is scheduled for July._

_Please read the rest of this README.md for a description of the driver, and see [CHANGELOG.md](./CHANGELOG.md) for migration guide._
...
First, source the `setup.bash` script for your ROS distribution in your terminal. For example, for ROS iron use:
...
Run the sample_capture_cpp launch script to test that everything is working.
...
This will start the `zivid_camera` driver node and the `sample_capture_cpp` node.
The `zivid_camera` driver will connect to the first available Zivid camera, and 
then `sample_capture_cpp` will trigger captures repeatedly.
...
### capture
[std_srvs/Trigger.srv](https://docs.ros.org/en/latest/api/std_srvs/html/srv/Trigger.html)
...
[zivid_interfaces/srv/CaptureAssistantSuggestSettings.srv](./zivid_camera/srv/CaptureAssistantSuggestSettings.srv)
