...
void recursivelyFillInUnsetWithCameraDefault(Node & node, const Zivid::CameraInfo & cameraInfo)
{
  if constexpr (
    ...
    node.forEach(
      [&cameraInfo](auto & child) { recursivelyFillInUnsetWithCameraDefault(child, cameraInfo); });
...
ZividSettingsType fillInUnsetWithCameraDefault(
  const ZividSettingsType & settings, const Zivid::CameraInfo & cameraInfo)
{
...
public:
  ...
  : name_(name),
    dr_server_(dr_server_mutex_, ros::NodeHandle(nh, name_)),
    config_(ConfigType::__getDefault__())
  {
    static_assert(
      std::is_same_v<ZividSettings, Zivid::Settings> ||
      std::is_same_v<ZividSettings, Zivid::Settings2D> ||
...
CaptureSettingsController<ZividSettingsType, SettingsConfigType, SettingsAcquisitionConfigType>::
...
{
  general_config_dr_server_ =
    std::make_unique<SettingsConfigTypeDRServer>(config_node_name_, nh, camera);
  ...
  for (std::size_t i = 0; i < num_acquisition_servers; i++) {
    acquisition_config_dr_servers_.push_back(
      std::make_unique<SettingsAcquisitionConfigTypeDRServer>(
...
template <
  typename ZividSettingsType, typename SettingsConfigType, typename SettingsAcquisitionConfigType>
CaptureSettingsController<ZividSettingsType, SettingsConfigType, SettingsAcquisitionConfigType>::
  ~CaptureSettingsController() = default;

template <
  typename ZividSettingsType, typename SettingsConfigType, typename SettingsAcquisitionConfigType>
ZividSettingsType CaptureSettingsController<
  ZividSettingsType, SettingsConfigType, SettingsAcquisitionConfigType>::zividSettings() const
{
...
template <
  typename ZividSettingsType, typename SettingsConfigType, typename SettingsAcquisitionConfigType>
void CaptureSettingsController<
  ZividSettingsType, SettingsConfigType,
  SettingsAcquisitionConfigType>::setZividSettings(const ZividSettingsType & settings)
{
...
template <
  typename ZividSettingsType, typename SettingsConfigType, typename SettingsAcquisitionConfigType>
std::size_t CaptureSettingsController<
  ZividSettingsType, SettingsConfigType,
...
template class CaptureSettingsController<
