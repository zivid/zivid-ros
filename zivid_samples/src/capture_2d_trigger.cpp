...
public:
  Capture2DTrigger() : Node("capture_2d_trigger"), start_time_(now())
  {
    ...
    if (rate_hz_ <= 0.0) {
      RCLCPP_WARN(
        get_logger(), "Parameter rate_hz must be positive. Falling back to default %.1f Hz.",
        default_rate_hz);
    ...
    while (rclcpp::ok()) {
      ...
      if (wait_for_service_timeout_s_ > 0.0) {
        ...
        if (elapsed.seconds() >= wait_for_service_timeout_s_) {
          RCLCPP_ERROR(
            get_logger(), "capture_2d service was not available after %.1f s. Shutting down.",
            wait_for_service_timeout_s_);
          rclcpp::shutdown();
          return false;
        }
  ...
  void onTimer()
  {
    ...
    capture_2d_client_->async_send_request(
      request, [this](rclcpp::Client<std_srvs::srv::Trigger>::SharedFuture future) {
        ...
        if (!response->success) {
          RCLCPP_WARN_THROTTLE(
            get_logger(), *get_clock(), 2000, "capture_2d failed: %s", response->message.c_str());
        }
      });
  }
