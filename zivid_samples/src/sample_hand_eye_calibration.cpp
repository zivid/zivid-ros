...
const std::map<std::string, HandEyeCalibrationMode> hand_eye_calibration_mode_map = {
  {"eye-in-hand", HandEyeCalibrationMode::EyeInHand},
  {"eye-to-hand", HandEyeCalibrationMode::EyeToHand},
};
...
public:
  ...
  : Node("hand_eye_calibration_node"),
    m_save_directory{
      std::filesystem::temp_directory_path() / "zivid_ros_sample_hand_eye_calibration"},
    m_calibration_mode{get_parameter_enum(*this, "hand_eye_mode", hand_eye_calibration_mode_map)},
    ...
    m_world_frame{declare_parameter<std::string>("world_frame", "map", read_only_parameter)},
    m_robot_frame{
      declare_parameter<std::string>("robot_frame", "robot_end_effector", read_only_parameter)},
      ...
      default_rclcpp_services_qos, m_callback_group)}
  {
    ...
    RCLCPP_INFO(
      ...
      m_calibration_mode == HandEyeCalibrationMode::EyeToHand ? "eye-to-hand" : "eye-in-hand",
  ...
  std::string get_instructions() const
  {
    return "  1. Repeat the following for a desired number of captures, e.g. 10-20:\n"
           ...
           m_robot_frame + "' with a valid transform from the world frame '" + m_world_frame +
           "'.\n"
           "    c. Call the '" +
...
private:
  ...
  Zivid::Calibration::Pose read_and_save_robot_transform(const std::string & file_path_pose) const
  {
    ...
    try {
      transform_msg =
        m_tf_buffer->lookupTransform(m_world_frame, m_robot_frame, tf2::TimePointZero);
    } catch (const tf2::TransformException & ex) {
      fatal_error(
        get_logger(), "Failed to lookup the transform between world frame '" + m_world_frame +
                        "' and robot frame '" + m_robot_frame + "': " + ex.what());
    }
